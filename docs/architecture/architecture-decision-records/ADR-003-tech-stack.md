...
**Nota de compatibilidade (verificada em set/2026):** o Next.js 16 renomeou `middleware.ts` para `proxy.ts` — a proteção de rotas via Auth.js deve usar esse novo nome de arquivo. Além disso, o pacote `next-auth@5.0.0-beta.25` ainda não publicou `peerDependencies` cobrindo Next.js 16 (só declara `^14 || ^15`), embora a comunidade confirme compatibilidade funcional. Instalação requer `legacy-peer-deps=true` (já configurado em `.npmrc` na raiz do projeto) até o pacote atualizar seus metadados — reavaliar quando `next-auth` sair de beta.
